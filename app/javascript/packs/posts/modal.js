$(document).on('turbolinks:load', () => {
  ...
  $("body").on("click", ".single-post-card, .single-post-list", function () {
    const posted_by = $(this).find('.post-content .posted-by').html();
    const post_heading = $(this).find('.post-content h3').html();
    const post_content = $(this).find('.post-content p').html();
    const interested = $(this).find('.post-content .interested').attr('href');
